...
## On-device Bank Editor (canvas.js) is GENERATED
...
Deploy = `scp src/canvas.js` to the module dir on the Move (re-read on every
editor open, no restart). The C side exposes it via the `editor` chain param
(`type:"canvas"`, `canvas_script:"canvas.js#bank_editor"`) in obxd_plugin.cpp.
